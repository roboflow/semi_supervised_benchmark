...
def run_job(script_path, dataset_url, gpu_id, suppress_output=False, force_rerun=False, model_name='yolov8n', skip_stac=False, num_reps=1, batch=16):
    """
...
def main(script, url_file, suppress_output=False, output_file=None, force_rerun=False, model_name='yolov8n', skip_stac=False, num_reps=1, batch=16):
    """
        ...
        for gpu in gpu_ids:
            ...
            if processes[gpu] is None and dataset_urls:
                ...
                new_proc = multiprocessing.Process(
                    target=run_job,
                    args=(script, url, gpu, suppress_output, force_rerun, model_name, skip_stac, num_reps, batch)
                )
                new_proc.start()
